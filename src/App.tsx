...
function StatCard({ icon, value, label, color = "#1677ff" }) {
  return (
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #f0f0f0",
      padding: "18px 20px", flex: 1, minWidth: 140, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
...
function Modal({ open, onClose, title, children }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000,
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: "28px 32px", minWidth: 380,
...
// ── Style tokens ──────────────────────────────────────────────────────────────
const inp  = { border: "1px solid #d9d9d9", borderRadius: 8, padding: "8px 12px", fontSize: 14,
  width: "100%", outline: "none", boxSizing: "border-box", fontFamily: "inherit", transition: "border 0.2s" };
const bBase = { border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 14,
  fontWeight: 600, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 };
const bP  = { ...bBase, background: "#1677ff", color: "#fff" };
const bG  = { ...bBase, background: "#52c41a", color: "#fff" };
const bO  = { ...bBase, background: "#fa8c16", color: "#fff" };
const bR  = { ...bBase, background: "#ff4d4f", color: "#fff" };
const bGr = { ...bBase, background: "#fff", color: "#595959", border: "1px solid #d9d9d9" };
const lbl  = { fontSize: 13, color: "#595959", marginBottom: 4, display: "block" };
const card = { background: "#fff", borderRadius: 14, border: "1px solid #f0f0f0",
...
export default function App() {
  ...
  const updateStock = useCallback((fn) => {
    setStock((prev) => {
      ...
      const obj  = {};
      next.forEach((s) => { obj[String(s.barcode).replace(/[^a-zA-Z0-9_-]/g, "_")] = cleanObj(s); });
  ...
  function confirmPick() {
    if (!barcode.trim()) {
      playSound("error"); setScanStatus("❌ กรุณากรอก Barcode"); setScanStatusColor("#ff4d4f"); return;
    }
    const pickQty = parseInt(String(qty), 10);
    if (isNaN(pickQty) || pickQty <= 0) {
      playSound("error"); setScanStatus("❌ กรุณาระบุจำนวน (Qty) ที่ถูกต้อง"); setScanStatusColor("#ff4d4f"); return;
    }
    const stockItem = stock.find((s) => s.barcode === barcode.trim());
    if (!stockItem) {
      playSound("error"); setScanStatus("❌ ไม่พบสินค้าในระบบ"); setScanStatusColor("#ff4d4f"); return;
    }
  ...
  function handleEditStock() {
    updateStock((prev) =>
      prev.map((s) => {
        ...
        if (s.barcode === key) { const { _originalBarcode, ...rest } = editStockItem; return rest; }
        return s;
      })
    );
  ...
  function exportExcel() {
    ...
    orders.forEach((o) => {
      ...
      if (o.completedAtRaw && o.createdAtRaw) {
        const s = Math.round((new Date(o.completedAtRaw) - new Date(o.createdAtRaw)) / 1000);
        dur = s < 60 ? s + " วินาที" : s < 3600 ? Math.round(s / 60) + " นาที" : Math.floor(s / 3600) + "ชม. " + Math.round((s % 3600) / 60) + "น.";
      }
      rows.push([o.pickNo, o.customer, o.barcode, o.product, o.location, o.required, o.picked || 0, o.status, o.createdAt, o.completedAt || "", dur]);
    });
  ...
  function downloadStockTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([["barcode","product","stock","location"],["8851234567890","สินค้า A",100,"A01-01"]]);
    ws["!cols"] = [{ wch: 20 },{ wch: 20 },{ wch: 10 },{ wch: 14 }];
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Stock"); XLSX.writeFile(wb, "stock_template.xlsx");
  }
  function downloadOrderTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([["pickNo","customer","barcode","product","location","required"],["PO001","ลูกค้า A","8851234567890","สินค้า A","A01-01",5]]);
    ws["!cols"] = [{ wch: 14 },{ wch: 16 },{ wch: 20 },{ wch: 20 },{ wch: 14 },{ wch: 12 }];
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Orders"); XLSX.writeFile(wb, "order_template.xlsx");
  }
    ...
    r.onload = (ev) => {
      try {
        ...
        const rows = raw.map((x, i) => {
          const row = type === "stock"
            ? { barcode: String(x.barcode || x.Barcode || x["บาร์โค้ด"] || "").trim(), product: String(x.product || x.Product || x["ชื่อสินค้า"] || "").trim(), stock: Number(x.stock || x.Stock || x["จำนวน"] || 0), location: String(x.location || x.Location || x["ที่เก็บ"] || "").trim() }
            : { pickNo: String(x.pickNo || x["Pick No"] || x.pick_no || "").trim(), customer: String(x.customer || x.Customer || x["ลูกค้า"] || "").trim(), barcode: String(x.barcode || x.Barcode || x["บาร์โค้ด"] || "").trim(), product: String(x.product || x.Product || x["สินค้า"] || x["ชื่อสินค้า"] || "").trim(), location: String(x.location || x.Location || x["ที่เก็บ"] || "").trim(), required: Number(x.required || x.Required || x["จำนวน"] || 1) };
          if (!row.barcode) errors.push(`แถว ${i + 2}: ไม่มี Barcode`);
          if (!row.product) errors.push(`แถว ${i + 2}: ไม่มีชื่อสินค้า`);
          return row;
        }).filter((x) => x.barcode);
        setXlsxPreview({ type, rows, errors });
    ...
    if (xlsxPreview.type === "stock") {
      updateStock((prev) => {
        ...
        xlsxPreview.rows.forEach((r) => {
          ...
          if (i >= 0) u[i] = { ...u[i], stock: u[i].stock + r.stock, location: r.location || u[i].location, product: r.product || u[i].product };
          else u.push(r);
        });
        return u;
      });
    ...
    <div style={{ background: "#f7f8fa", minHeight: "100vh", fontFamily: "'Noto Sans Thai',sans-serif", color: "#262626" }}>
      ...
      {alertMsg && (
        <div style={{ position: "fixed", top: 20, right: 20, zIndex: 2000, background: alertBg, color: "#fff",
          ...
          boxShadow: "0 4px 16px rgba(0,0,0,0.18)", animation: "fadein 0.3s", maxWidth: 380 }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #f0f0f0", padding: "0 32px",
        display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 100, height: 58 }}>
        ...
        <button style={bGr} onClick={backupData}>💾 Backup</button>
        <button style={bGr} onClick={() => document.getElementById("restore-file").click()}>🔄 Restore</button>
        <button style={bR}  onClick={() => setConfirmClear(true)}>🗑️ ล้างข้อมูลวันนี้</button>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <button onClick={() => setAutoConfirm((v) => !v)}
              style={{ ...bP, background: autoConfirm ? "#52c41a" : "#8c8c8c", fontSize: 12, padding: "4px 12px" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
            <div style={{ flex: "1 1 180px" }}>
              ...
              <input style={inp} value={pickNo} onChange={(e) => setPickNo(e.target.value)}
                placeholder="สแกนหรือกรอก Pick No"
                onKeyDown={(e) => e.key === "Enter" && barcodeRef.current?.focus()} />
            </div>
            <div style={{ flex: "1 1 220px" }}>
              <label style={lbl}>Barcode <span style={{ color: "#8c8c8c", fontWeight: 400 }}>(กด Enter เพื่อบันทึก)</span></label>
              <input ref={barcodeRef} style={{ ...inp, border: "2px solid #40a9ff", background: "#f0f9ff" }}
            ...
            <div style={{ flex: "0 0 100px" }}>
              ...
              <input style={{ ...inp, textAlign: "center" }} type="number" min="1" value={qty}
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 220 }}>
              <input style={{ ...inp, maxWidth: 320 }} value={searchOrder} onChange={(e) => setSearchOrder(e.target.value)}
            ...
            <button style={bO} onClick={exportExcel}>⬇️ Export Excel</button>
          </div>
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  return (
                    <tr key={o.id} style={{ borderBottom: "1px solid #f5f5f5", background: isOver ? "#fff1f0" : "transparent" }}>
                      ...
                      <td style={{ padding: "10px 12px", textAlign: "center" }}>
                        {fmtDur(o) ?? <span style={{ color: "#d9d9d9", fontSize: 12 }}>-</span>}
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => setEditOrderItem({ ...o })} style={{ ...bGr, padding: "4px 10px", fontSize: 12 }}>✏️</button>
                          <button onClick={() => { updateOrders((prev) => prev.filter((x) => x.id !== o.id)); showAlert("ลบ Order แล้ว"); }}
                            style={{ ...bR, padding: "4px 10px", fontSize: 12 }}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
          <div style={card}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={{ ...bGr, fontSize: 12, padding: "5px 12px" }} onClick={downloadStockTemplate}>📄 Template</button>
                <input type="file" accept=".xlsx,.xls,.csv" id="stock-xlsx" style={{ display: "none" }} onChange={(e) => { const f = e.target.files[0]; if (f) parseXlsx(f, "stock"); e.target.value = ""; }} />
                <button style={{ ...bG, fontSize: 12, padding: "5px 12px" }} onClick={() => document.getElementById("stock-xlsx").click()}>📤 Upload Excel</button>
              </div>
            </div>
            <div style={{ border: "2px dashed #91d5ff", borderRadius: 8, padding: "10px 14px", marginBottom: 14, background: "#f0f9ff", fontSize: 12, color: "#0958d9", textAlign: "center", cursor: "pointer" }}
              onClick={() => document.getElementById("stock-xlsx").click()}
              onDragOver={(e) => e.preventDefault()}
              onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) parseXlsx(f, "stock"); }}>
          ...
          <div style={card}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button style={{ ...bGr, fontSize: 12, padding: "5px 12px" }} onClick={downloadOrderTemplate}>📄 Template</button>
                <input type="file" accept=".xlsx,.xls,.csv" id="order-xlsx" style={{ display: "none" }} onChange={(e) => { const f = e.target.files[0]; if (f) parseXlsx(f, "order"); e.target.value = ""; }} />
                <button style={{ ...bO, fontSize: 12, padding: "5px 12px" }} onClick={() => document.getElementById("order-xlsx").click()}>📤 Upload Excel</button>
              </div>
            </div>
            <div style={{ border: "2px dashed #ffd591", borderRadius: 8, padding: "10px 14px", marginBottom: 14, background: "#fffbe6", fontSize: 12, color: "#874d00", textAlign: "center", cursor: "pointer" }}
              onClick={() => document.getElementById("order-xlsx").click()}
              onDragOver={(e) => e.preventDefault()}
              onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) parseXlsx(f, "order"); }}>
              📂 คลิกหรือลากไฟล์ Excel / CSV มาวางที่นี่<br />
              <span style={{ color: "#8c8c8c" }}>รองรับคอลัมน์: pickNo, customer, barcode, product, location, required</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div><label style={lbl}>Pick No (ไม่ต้องใส่ก็ได้)</label>
                <input style={inp} value={addOrderForm.pickNo} onChange={(e) => setAddOrderForm((f) => ({ ...f, pickNo: e.target.value }))} placeholder="Pick No" /></div>
              <div><label style={lbl}>ชื่อลูกค้า</label>
                <input style={inp} value={addOrderForm.customer} onChange={(e) => setAddOrderForm((f) => ({ ...f, customer: e.target.value }))} placeholder="ชื่อลูกค้า" /></div>
              <div>
                <label style={lbl}>Barcode *</label>
                <input style={{ ...inp, border: addOrderForm.barcode && stock.find((s) => s.barcode === addOrderForm.barcode.trim()) ? "2px solid #52c41a" : inp.border }}
                  value={addOrderForm.barcode}
                  onChange={(e) => { const bc = e.target.value; const found = stock.find((s) => s.barcode === bc.trim()); setAddOrderForm((f) => ({ ...f, barcode: bc, product: found ? found.product : f.product, location: found ? found.location : f.location })); }}
                  placeholder="Barcode *" />
                {addOrderForm.barcode && (() => {
                  ...
                  return found
                    ...
                    : addOrderForm.barcode.length > 3 ? <div style={{ marginTop: 4, fontSize: 12, color: "#fa8c16" }}>⚠️ ไม่พบ Barcode นี้ใน Stock</div> : null;
                })()}
              </div>
              <div><label style={lbl}>ชื่อสินค้า *</label>
                <input style={inp} value={addOrderForm.product} onChange={(e) => setAddOrderForm((f) => ({ ...f, product: e.target.value }))} placeholder="ชื่อสินค้า *" /></div>
              <div><label style={lbl}>Location</label>
                <input style={inp} value={addOrderForm.location} onChange={(e) => setAddOrderForm((f) => ({ ...f, location: e.target.value }))} placeholder="Location" /></div>
              <div><label style={lbl}>จำนวนที่ต้องหยิบ</label>
                <input style={inp} type="number" min="1" value={addOrderForm.required} onChange={(e) => setAddOrderForm((f) => ({ ...f, required: e.target.value }))} placeholder="จำนวน" /></div>
              <button style={{ ...bO, marginTop: 4, width: "100%", justifyContent: "center" }} onClick={handleAddOrder}>➕ เพิ่ม Order</button>
            </div>
          </div>
        </div>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <input style={{ ...inp, maxWidth: 280 }} value={stockSearch} onChange={(e) => setStockSearch(e.target.value)} placeholder="ค้นหา Stock..." />
            </div>
            <button style={bR} onClick={() => { if (window.confirm("ลบ Stock ทั้งหมด?")) { updateStock([]); showAlert("ลบ Stock ทั้งหมดแล้ว"); } }}>🗑️ ลบทั้งหมด</button>
          </div>
          <div style={{ overflowX: "auto" }}>
                ...
                {filteredStock.map((s) => (
                  <tr key={s.barcode} style={{ borderBottom: "1px solid #f5f5f5" }}>
                    ...
                    <td style={{ padding: "10px 14px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setEditStockItem({ ...s, _originalBarcode: s.barcode })} style={{ ...bP, padding: "5px 14px", fontSize: 13 }}>✏️ แก้ไข</button>
                        <button onClick={() => { updateStock((prev) => prev.filter((x) => x.barcode !== s.barcode)); showAlert("ลบสินค้าแล้ว"); }} style={{ ...bR, padding: "5px 14px", fontSize: 13 }}>🗑️ ลบ</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {filteredStock.length === 0 && <tr><td colSpan={5} style={{ textAlign: "center", padding: 30, color: "#8c8c8c" }}>ไม่พบสินค้า</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      <Modal open={!!editStockItem} onClose={() => setEditStockItem(null)} title="แก้ไขสต็อก">
        {editStockItem && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[["barcode","Barcode"],["product","ชื่อสินค้า"],["stock","จำนวน","number"],["location","Location"]].map(([k, lb, tp]) => (
              <div key={k}><label style={lbl}>{lb}</label>
                <input style={inp} type={tp || "text"} value={editStockItem[k]}
                  onChange={(e) => setEditStockItem((f) => ({ ...f, [k]: tp === "number" ? Number(e.target.value) : e.target.value }))} /></div>
            ))}
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              <button style={{ ...bP, flex: 1 }} onClick={handleEditStock}>💾 บันทึก</button>
              <button style={{ ...bGr, flex: 1 }} onClick={() => setEditStockItem(null)}>ยกเลิก</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={!!editOrderItem} onClose={() => setEditOrderItem(null)} title="แก้ไข Order">
        {editOrderItem && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[["pickNo","Pick No"],["customer","ลูกค้า"],["barcode","Barcode"],["product","สินค้า"],["location","Location"],["required","จำนวนที่ต้องหยิบ","number"],["picked","หยิบแล้ว","number"]].map(([k, lb, tp]) => (
              <div key={k}><label style={lbl}>{lb}</label>
                <input style={inp} type={tp || "text"} value={editOrderItem[k] ?? ""}
                  onChange={(e) => setEditOrderItem((f) => ({ ...f, [k]: tp === "number" ? Number(e.target.value) : e.target.value }))} /></div>
            ))}
            <div><label style={lbl}>Status</label>
              <select style={inp} value={editOrderItem.status} onChange={(e) => setEditOrderItem((f) => ({ ...f, status: e.target.value }))}>
                <option>PENDING</option><option>COMPLETE</option>
              </select></div>
            {editOrderItem.status === "COMPLETE" && !editOrderItem.completedAt && (
              <div style={{ background: "#f6ffed", border: "1px solid #b7eb8f", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#389e0d" }}>
                ✅ ระบบจะบันทึกเวลา Completed At อัตโนมัติเมื่อกดบันทึก
              </div>
            )}
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              <button style={{ ...bP, flex: 1 }} onClick={handleEditOrder}>💾 บันทึก</button>
              <button style={{ ...bGr, flex: 1 }} onClick={() => setEditOrderItem(null)}>ยกเลิก</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* Excel Preview Modal */}
      <Modal open={!!xlsxPreview} onClose={() => setXlsxPreview(null)}
        title={xlsxPreview?.type === "stock" ? "📤 ตรวจสอบข้อมูล Stock ก่อนนำเข้า" : "📤 ตรวจสอบข้อมูล Order ก่อนนำเข้า"}>
        {xlsxPreview && (
          <div>
            ...
            <div style={{ maxHeight: 240, overflowY: "auto", border: "1px solid #f0f0f0", borderRadius: 8, marginBottom: 16 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead><tr style={{ background: "#fafafa", position: "sticky", top: 0 }}>
                  {(xlsxPreview.type === "stock" ? ["Barcode","Product","Stock","Location"] : ["Pick No","Customer","Barcode","Product","Location","Required"]).map((h) => (
                    <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontWeight: 600, color: "#595959" }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {xlsxPreview.rows.slice(0, 50).map((r, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #f5f5f5" }}>
                      {(xlsxPreview.type === "stock" ? [r.barcode,r.product,r.stock,r.location] : [r.pickNo,r.customer,r.barcode,r.product,r.location,r.required]).map((v, j) => (
                        <td key={j} style={{ padding: "6px 10px" }}>{v}</td>
                      ))}
                    </tr>
                  ))}
                  {xlsxPreview.rows.length > 50 && <tr><td colSpan={6} style={{ padding: "8px 10px", color: "#8c8c8c", fontSize: 12, textAlign: "center" }}>...แสดง 50 แถวแรก จากทั้งหมด {xlsxPreview.rows.length} แถว</td></tr>}
                </tbody>
              </table>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button style={{ ...bG, flex: 1, justifyContent: "center" }} onClick={confirmXlsxImport}>✅ ยืนยันนำเข้า {xlsxPreview.rows.length} รายการ</button>
              <button style={{ ...bGr, flex: 1, justifyContent: "center" }} onClick={() => setXlsxPreview(null)}>ยกเลิก</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={confirmClear} onClose={() => setConfirmClear(false)} title="⚠️ ล้างข้อมูลวันนี้">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button style={{ ...bR, flex: 1 }} onClick={() => {
          ...
          <button style={{ ...bGr, flex: 1 }} onClick={() => setConfirmClear(false)}>ยกเลิก</button>
        </div>
      </Modal>
